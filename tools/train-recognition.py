...
from contest.recognition.transforms import get_train_transforms, get_val_transforms
...
def train(model, criterion, optimizer, train_dataloader, logger, device, scheduler):
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train(model, criterion, optimizer, train_dataloader, logger, device, scheduler)
        ...
        if val_loss < best_val_loss:
            ...
            logger.info(f"Val loss: {val_loss:.5f} (best)")
        else:
            logger.info(f"Val loss: {val_loss:.5f} (best {best_val_loss:.5f})")
